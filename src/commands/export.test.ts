...
async function makeWorkspaceWithMissingDependency(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "sklock-export-"));
...
describe("exportCommand", () => {
  ...
  it("exits non-zero instead of exporting graphs with missing dependencies", async () => {
    ...
    await expect(exportCommand({ root, format: "mermaid" })).rejects.toThrow("exit:2");
